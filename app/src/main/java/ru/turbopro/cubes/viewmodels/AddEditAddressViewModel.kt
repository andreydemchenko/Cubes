...
import ru.turbopro.cubes.getAddressId
import ru.turbopro.cubes.isPhoneValid
import ru.turbopro.cubes.isZipCodeValid
...
class AddEditAddressViewModel(application: Application) : AndroidViewModel(application) {
	...
	private val _addAddressStatus = MutableLiveData<AddProductObjectStatus?>()
	val addProductAddressStatus: LiveData<AddProductObjectStatus?> get() = _addAddressStatus
	...
	private fun updateAddress() {
		viewModelScope.launch {
			if (newAddressData.value != null && _addressData.value != null) {
				_addAddressStatus.value = AddProductObjectStatus.ADDING
				val updateRes = async {
				...
				if (res is Success) {
					...
					_addAddressStatus.value = AddProductObjectStatus.DONE
				} else {
					Log.d(TAG, "onUpdate: Some error occurred!")
					_addAddressStatus.value = AddProductObjectStatus.ERR_ADD
					if (res is Error)
