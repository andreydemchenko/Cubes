package ru.turbopro.cubes.data.source.remote
...
class ProductsRemoteDataSource : ProductDataSource {
	...
	override suspend fun updateProduct(evData: Product) {
		val resRef =
			productsCollectionRef().whereEqualTo(PRODUCT_ID_FIELD, evData.productId).get().await()
		if (!resRef.isEmpty) {
			val docId = resRef.documents[0].id
			productsCollectionRef().document(docId).set(evData.toHashMap()).await()
		} else {
			Log.d(TAG, "onUpdateProduct: product with id: $evData.productId not found!")
		}
	}
