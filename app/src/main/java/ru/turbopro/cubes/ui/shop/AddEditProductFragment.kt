...
import ru.turbopro.cubes.data.utils.AddEditProductErrors
import ru.turbopro.cubes.data.utils.ShoeColors
import ru.turbopro.cubes.data.utils.ShoeSizes
import ru.turbopro.cubes.data.utils.StoreProductDataStatus
import ru.turbopro.cubes.databinding.FragmentAddEditProductBinding
import ru.turbopro.cubes.ui.AddProductViewErrors
import ru.turbopro.cubes.ui.MyOnFocusChangeListener
import ru.turbopro.cubes.ui.main.AddImagesAdapter
import ru.turbopro.cubes.viewmodels.AddEditProductViewModel
...
class AddEditProductFragment : Fragment() {
	...
	private fun setObservers() {
		...
		viewModel.addEditProductErrors.observe(viewLifecycleOwner) { status ->
			when (status) {
				AddEditProductErrors.ADDING -> {
					binding.loaderLayout.loaderFrameLayout.visibility = View.VISIBLE
					binding.loaderLayout.circularLoader.showAnimationBehavior
				}
				AddEditProductErrors.ERR_ADD_IMG -> {
					setAddProductErrors(getString(R.string.add_error_img_upload))
				}
				AddEditProductErrors.ERR_ADD -> {
					setAddProductErrors(getString(R.string.add_product_insert_error))
				}
				AddEditProductErrors.NONE -> {
					binding.loaderLayout.loaderFrameLayout.visibility = View.GONE
					binding.loaderLayout.circularLoader.hideAnimationBehavior
				}
			}
	...
	private fun setViews() {
		...
		binding.addProBtn.setOnClickListener {
			...
			if (viewModel.errorStatus.value == AddProductViewErrors.NONE) {
				viewModel.addEditProductErrors.observe(viewLifecycleOwner) { err ->
					if (err == AddEditProductErrors.NONE) {
	...
	private fun modifyErrors(err: AddProductViewErrors) {
		when (err) {
			...
			AddProductViewErrors.EMPTY -> {
				binding.addProErrorTextView.visibility = View.VISIBLE
				binding.addProErrorTextView.text = getString(R.string.add_error_string)
			}
