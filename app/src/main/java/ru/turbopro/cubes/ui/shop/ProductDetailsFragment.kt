...
import ru.turbopro.cubes.data.utils.AddProductObjectStatus
...
import ru.turbopro.cubes.databinding.FragmentProductDetailsBinding
import ru.turbopro.cubes.ui.AddProductItemErrors
import ru.turbopro.cubes.ui.DotsIndicatorDecoration
import ru.turbopro.cubes.viewmodels.ProductViewModel
...
class ProductDetailsFragment : Fragment() {
			...
			binding.proDetailsAddCartBtn.setOnClickListener {
				if (viewModel.isItemInCart.value == true) {
					...
					if (viewModel.errorStatus.value?.isEmpty() == true) {
						viewModel.addProductItemStatus.observe(viewLifecycleOwner) { status ->
							if (status == AddProductObjectStatus.DONE) {
	...
	private fun setImagesView() {
		if (context != null) {
			...
			if (viewModel.getCountOfProductsList() > 1) binding.proDetailsImagesRecyclerview.addItemDecoration(itemDecoration)
